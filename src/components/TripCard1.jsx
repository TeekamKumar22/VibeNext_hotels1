...
const TripCard = () => {
  ...
  useEffect(() => {
    ...
    .then(res => {
      ...
      const sliced = data.slice(10, 13); // Only take first 3 hotels
  ...
  return (
    <div className="flex flex-col items-center w-full px-1 sm:px-6 lg:px-8 py-1">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-6xl">
        {roomDetails.map((post, index) => (
          <article
            key={post._id}
            ref={el => cardRefs.current[index] = el}
            className={`relative isolate flex flex-col justify-end overflow-hidden rounded-lg aspect-[3/2] shadow-lg w-[24rem] max-w-[22.3rem] mx-auto transform transition-all duration-700
